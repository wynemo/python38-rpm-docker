...
RUN bash -c "find /tmp/installdir/usr/local/lib/python3.8/ -type d -name __pycache__ -exec rmdir {} \;"
RUN bash -c "find /tmp/installdir/usr/local/lib/python3.8/ -name "*.pyc" -exec rm -f {} \;"
RUN bash -c "source /etc/profile.d/rvm.sh;\
    fpm -s dir -t rpm -n python38 -v ${BUILD_VER} -C /tmp/installdir \
    ...
    -d 'readline' \
    -d 'gdbm' \
    --directories=/usr/local/lib/python3.8/ \
    --directories=/usr/local/include/python3.8m/ \
    usr/local"
